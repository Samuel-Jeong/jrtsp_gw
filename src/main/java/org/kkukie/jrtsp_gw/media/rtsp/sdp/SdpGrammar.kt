...
object SdpGrammar {
    ...
    private val CANDIDATES =
        """candidate:(\S*) (\d*) (\S*) (\d*) (\S*) (\d*) typ (\S*)(?: raddr (\S*) rport (\d*))?(?: tcptype (\S*))?(?: generation (\d*))?(?: network-id (\d*))?(?: network-cost (\d*))?""".toRegex()
    ...
    fun tryParseCandidates(string: String): SdpCandidates? {
        ...
        return SdpCandidates(
            ...
            port.toLong(),
            type,
            toOptionalString(raddr),
            toOptionalLong(rport),
            toOptionalString(tcptype),
            toOptionalLong(generation),
            toOptionalLong(networkId),
            toOptionalLong(networkCost)
        )
    }
    ...
    private val IMAGEATTRS =
        """imageattr:(\d+|\*)[\s\t]+(send|recv)[\s\t]+(\*|[\S+](?:[\s\t]+[\S+])*)(?:[\s\t]+(recv|send)[\s\t]+(\*|[\S+](?:[\s\t]+[\S+])*))?""".toRegex()
    ...
    private val SIMULCAST =
        """simulcast:(send|recv) ([a-zA-Z0-9\-_~;,]+)(?:\s?(send|recv) ([a-zA-Z0-9\-_~;,]+))?$""".toRegex()
        ...
        when (type) {
            ...
            'a' -> {
                ...
                if (tryRtcpfbtrrint != null) {
                    if (lastMedia != null) lastMedia.rtcpFbTrrInt.add(tryRtcpfbtrrint) else rtcpFbTrrInt.add(
                        tryRtcpfbtrrint
                    )
                    return
                }
                ...
                if (tryExtmapallowmixed != null) {
                    if (lastMedia != null) lastMedia.extmapAllowMixed = tryExtmapallowmixed else extmapAllowMixed =
                        tryExtmapallowmixed
                    return
                }
                ...
                if (tryConnectiontype != null) {
                    if (lastMedia != null) lastMedia.connectionType = tryConnectiontype else connectionType =
                        tryConnectiontype
                    return
                }
                ...
                if (tryEndofcandidates != null) {
                    if (lastMedia != null) lastMedia.endOfCandidates = tryEndofcandidates else endOfCandidates =
                        tryEndofcandidates
                    return
                }
                val tryRemotecandidates = SdpGrammar.tryParseRemotecandidates(value)
                if (tryRemotecandidates != null) {
                    if (lastMedia != null) lastMedia.remoteCandidates = tryRemotecandidates else remoteCandidates =
                        tryRemotecandidates
                    return
                }
                ...
                if (tryMaxmessagesize != null) {
                    if (lastMedia != null) lastMedia.maxMessageSize = tryMaxmessagesize else maxMessageSize =
                        tryMaxmessagesize
                    return
                }
                ...
                if (tryBfcpfloorctrl != null) {
                    if (lastMedia != null) lastMedia.bfcpFloorCtrl = tryBfcpfloorctrl else bfcpFloorCtrl =
                        tryBfcpfloorctrl
                    return
                }
