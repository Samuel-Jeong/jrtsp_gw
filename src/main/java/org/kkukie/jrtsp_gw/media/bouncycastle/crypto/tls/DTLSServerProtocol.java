    ...
    extends DTLSProtocol
{
    ...
    private static final Logger logger = LoggerFactory.getLogger(DTLSServerProtocol.class);
...
//        ProtocolVersion recordLayerVersion = recordLayer.getReadVersion();
        ...
        TlsCertificate serverTlsCertificate = null;
        ...
        else
        {
            ...
            serverTlsCertificate = state.serverCredentials.getCertificate();
            byte[] certificateBody = generateCertificate(serverTlsCertificate);
            handshake.sendMessage(HandshakeType.certificate, certificateBody);
        }

        // TODO[RFC 3546] Check whether empty certificates is possible, allowed, or excludes CertificateStatus
        if (serverTlsCertificate == null || serverTlsCertificate.isEmpty())
        {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    /*
                     * RFC 5246 If no suitable tlsCertificate is available, the client MUST send a
                     * tlsCertificate message containing no certificates.
                     * 
                ...
                notifyClientCertificate(state, TlsCertificate.EMPTY_CHAIN);
            }
        }
        ...
        /*
         * RFC 5246 7.4.8 This message is only sent following a client tlsCertificate that has signing
...
//        }
        ...
        state.server.setSrtpMasterKey(securityParameters.getMasterSecret());
//        state.server.prepareSrtpSharedSecret();
    ...
    protected void notifyClientCertificate(ServerHandshakeState state, TlsCertificate clientTlsCertificate)
        throws IOException
    {
        if (state.certificateRequest == null)
        {
        ...
        if (state.clientTlsCertificate != null)
        {
            throw new TlsFatalAlert(AlertDescription.unexpected_message);
        }

        state.clientTlsCertificate = clientTlsCertificate;

        if (clientTlsCertificate.isEmpty())
        {
            state.keyExchange.skipClientCredentials();
        }
        else
        {
            ...
            state.clientCertificateType = TlsUtils.getClientCertificateType(clientTlsCertificate,
                state.serverCredentials.getCertificate());

            state.keyExchange.processClientCertificate(clientTlsCertificate);
        }

        /*
         * RFC 5246 7.4.6. If the client does not send any certificates, the server MAY at its
         * discretion either continue the handshake without client authentication, or respond with a
         * fatal handshake_failure alert. Also, if some aspect of the tlsCertificate chain was
         * unacceptable (e.g., it was not signed by a known, trusted CA), the server MAY at its
         * discretion either continue the handshake (considering the client unauthenticated) or send
         * a fatal alert.
         */
        state.server.notifyClientCertificate(clientTlsCertificate);
    }
        ...
        throws IOException
    {
        ...
        TlsCertificate clientTlsCertificate = TlsCertificate.parse(buf);

        TlsProtocol.assertEmpty(buf);

        notifyClientCertificate(state, clientTlsCertificate);
    }
        ...
        throws IOException
    {
        ...
        try
        {
            ...
            org.kkukie.jrtsp_gw.media.bouncycastle.asn1.x509.Certificate x509Cert = state.clientTlsCertificate.getCertificateAt(0);
    ...
    protected static class ServerHandshakeState
    {
        ...
        TlsCertificate clientTlsCertificate = null;
    }
}
