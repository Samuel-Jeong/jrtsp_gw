...
{
    private static final Logger logger = LoggerFactory.getLogger(DTLSClientProtocol.class);
        ...
        throws IOException
    {
        ...
        {
            serverTlsCertificate = processServerCertificate(state, serverMessage.getBody());
            serverMessage = handshake.receiveMessage();
        }
        else
        {
            // Okay, TlsCertificate is optional
        ...
        if (state.certificateRequest != null)
        {
            ...
            /*
             * RFC 5246 If no suitable tlsCertificate is available, the client MUST send a tlsCertificate
            ...
            {
                clientTlsCertificate = state.clientCredentials.getCertificate();
            }
            if (clientTlsCertificate == null)
            {
                clientTlsCertificate = TlsCertificate.EMPTY_CHAIN;
            }

            byte[] certificateBody = generateCertificate(clientTlsCertificate);
        ...
        state.client.notifyHandshakeComplete();
        state.client.setSrtpMasterKey(securityParameters.getMasterSecret());
        return new DTLSTransport(recordLayer);
    }
...
//        reportServerVersion(state, server_version);
    ...
    protected TlsCertificate processServerCertificate(ClientHandshakeState state, byte[] body)
        throws IOException
    {
        ByteArrayInputStream buf = new ByteArrayInputStream(body);

        TlsCertificate serverTlsCertificate = TlsCertificate.parse(buf);

        TlsProtocol.assertEmpty(buf);

        state.keyExchange.processServerCertificate(serverTlsCertificate);
        state.authentication = state.client.getAuthentication();
        state.authentication.notifyServerCertificate(serverTlsCertificate);
