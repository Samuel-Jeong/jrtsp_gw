...
public class RtcpHandler implements PacketHandler {
    ...
    public RtcpHandler(final Scheduler scheduler, final RtpStatistics statistics, String mediaType) {
    ...
    public byte[] handle(byte[] packet, int dataLength, int offset, InetSocketAddress localPeer, InetSocketAddress remotePeer)
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("\nRECEIVED RTCP [{}]", rtcpPacket.toString());
        }
    ...
    public void sendRtcpRawPacket (byte[] data) throws IOException {
        ...
        if (this.channel != null && channel.isOpen() && channel.isConnected()) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("SENDING RTCP [dataLen={}]", data.length);
            }

        } else {
            if (logger.isDebugEnabled()) {
                if (this.channel == null) {
                    logger.debug("RtcpHandler: Channel is null.");
                } else if (!this.channel.isOpen()) {
                    logger.debug("RtcpHandler: Channel is closed.");
                } else if (!this.channel.isConnected()) {
                    logger.debug("RtcpHandler: Channel is disconnected.");
                }
                logger.debug("RtcpHandler: Could not send raw rtcp packet.");
            }
        }
    }
        ...
        if (this.channel != null && channel.isOpen() && channel.isConnected()) {
            ...
            if (channel.isOpen() && channel.isConnected()) {
                ...
                if (logger.isDebugEnabled()) {
                    ...
                    logger.debug("Could not send {} packet.", type);
                }
                return;
            }
            ...
            if (logger.isDebugEnabled()) {
                ...
                logger.debug("RtcpHandler: Could not send {} packet.", type);
            }
        }
    }
    ...
    private class TxTask implements Runnable {
        ...
        private void onExpire() throws IOException {
            ...
            switch (this.packetType) {
                ...
                default:
                    logger.warn("RtcpHandler: Unknown scheduled event type!");
                    break;
            }
        }
