...
package org.kkukie.jrtsp_gw.media.core.network.netty.channel;
...
/**
 * Implementation if {@link NettyNetworkChannelFsm} that uses Asynchronous API.
 * 
...
public class AsyncNettyNetworkChannelFsm extends AbstractNettyNetworkChannelFsm {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AsyncNettyNetworkChannelFsm.class);
    ...
    private final class OpenCallback implements FutureCallback<Channel> {
        ...
        public void onSuccess(Channel result) {
            if(logger.isTraceEnabled()) {
                logger.trace("Channel opened successfully");
            }
        ...
        public void onFailure(Throwable t) {
            logger.warn("Channel could not be opened, so will be terminated prematurely.", t);
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            if (future.isSuccess()) {
                if(logger.isDebugEnabled()) {
                    logger.debug("Channel is bound to " + future.channel().localAddress().toString());
                }
                fire(NettyNetworkChannelEvent.BOUND, this.context);
                this.context.getCallback().onSuccess(null);
            } else {
                logger.warn("Could not bind channel to " + globalContext.getLocalAddress() + ". Channel will be closed.", future.cause());
                fire(NettyNetworkChannelEvent.CLOSE, this.context);
    ...
    private final class ConnectCallback implements ChannelFutureListener {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            if (future.isSuccess()) {
                if(logger.isDebugEnabled()) {
                    logger.debug("Channel is connected to " + future.channel().remoteAddress().toString());
                }
                fire(NettyNetworkChannelEvent.CONNECTED, this.context);
                this.context.getCallback().onSuccess(null);
            } else {
                logger.warn("Could not connect channel to " + globalContext.getRemoteAddress() + ". Channel will be closed.", future.cause());
                fire(NettyNetworkChannelEvent.CLOSE, this.context);
                this.context.getCallback().onFailure(future.cause());
            }
        }
    ...
    private final class DisconnectCallback implements ChannelFutureListener {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            if (future.isSuccess()) {
                if(logger.isDebugEnabled()) {
                    logger.debug("Channel disconnected from " + future.channel().remoteAddress().toString());
                }
                fire(NettyNetworkChannelEvent.DISCONNECTED, this.context);
                this.context.getCallback().onSuccess(null);
            } else {
                logger.warn("Could not disconnect channel from " + globalContext.getRemoteAddress() + ". Channel will be closed.", future.cause());
                fire(NettyNetworkChannelEvent.CLOSE, this.context);
                this.context.getCallback().onFailure(future.cause());
            }
        }
    ...
    private final class CloseCallback implements ChannelFutureListener {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            if (future.isSuccess()) {
                if(logger.isDebugEnabled()) {
                    logger.debug("Channel closed in elegant manner");
                }
                this.context.getCallback().onSuccess(null);
            } else {
                logger.warn("Channel could not be closed properly", future.cause());
                this.context.getCallback().onFailure(future.cause());
            }
