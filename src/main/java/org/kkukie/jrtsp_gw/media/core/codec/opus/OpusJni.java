...
package org.kkukie.jrtsp_gw.media.core.codec.opus;
...
public class OpusJni {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OpusJni.class);
    ...
    static {
        ...
        if (libraryName != null) {
            try {
                ...
                logger.error("Failed to load native Opus library. " + e.getMessage());
                throw e;
            }
        } else {
            logger.error("Native Opus library parameter has not been defined (restcomm.opus.library).");
        }
    }
